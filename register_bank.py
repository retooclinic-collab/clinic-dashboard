...
- 기존 connectedId에 하나(0081)·부산(0032) 은행계좌를 add_account로 추가
- 아이디/비밀번호 로그인 방식 (loginType="1")
- clientType 기본 "B"(법인/기업뱅킹). 개인계좌면 BANK_CLIENT_TYPE=P.
...
필요 시크릿:
  ...
  BANK_HANA_ID,  BANK_HANA_PW      (하나 기업뱅킹 아이디/비번)
  BANK_BUSAN_ID, BANK_BUSAN_PW     (부산 기업뱅킹 아이디/비번)
"""
import os, json
...
CLIENT_TYPE  = os.environ.get("BANK_CLIENT_TYPE", "B")  # B=법인/기업뱅킹(기본), P=개인

SVC = ServiceType.PRODUCT if ENV in ("api","prod","product") else ServiceType.DEMO

# (기관코드, 표시이름, ID시크릿, PW시크릿)
BANKS = [
...
def main():
    codef = make_codef()
    print("clientType:", CLIENT_TYPE, "(B=법인/기업뱅킹)", flush=True)
